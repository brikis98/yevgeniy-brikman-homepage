...
1. Create a `_config.dev.yml` file locally which contains a dev value for `algolia_search_api_key` (an API key that
   allows access from localhost, which the production one does not). DO NOT CHECK THIS FILE INTO VERSION CONTROL.
1. Just the first time: `bundle install`.
1. To build the site and serve it: `bundle exec jekyll serve --drafts --limit 100 --config _config.yml,_config.dev.yml`.
